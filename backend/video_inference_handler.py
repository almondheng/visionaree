...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if content_type.startswith("multipart/form-data"):
            ...
            video_data, original_filename, user_prompt = parse_multipart_video(
                body, content_type
            )
            if not user_prompt:
                user_prompt = headers.get("x-user-prompt")
            logger.info(
                f"Multipart upload detected: filename={original_filename}, user_prompt={user_prompt}"
            )
        elif (
            ...
            if not default_filename:
                ...
                default_filename = content_type_to_extension.get(
            ...
            user_prompt = headers.get("x-user-prompt")
            logger.info(
                f"Binary upload detected: content-type={content_type}, filename={original_filename}, user_prompt={user_prompt}"
            )
        else:
            return create_error_response(
                400,
                f"Unsupported content type: {content_type}. Please use multipart/form-data or a supported video MIME type.",
            )
        ...
        logger.info(
            f"Extracted video data: {len(video_data)} bytes, filename: {original_filename}"
        )
        ...
        if len(video_data) > max_size:
            return create_error_response(
                413, f"File too large. Maximum size is {max_size // (1024*1024)}MB"
            )

        # Process the video and run Bedrock inference
        video_format = format_validation.get("format", ".mp4").lstrip(
            "."
        )  # Remove leading dot
        inference_result = process_video_for_inference(
            video_data, original_filename, video_format, user_prompt
        )
    ...
    try:
        ...
        for part in parts:
            ...
            if 'name="' in content_disposition:
                ...
                field_name = (
                    content_disposition.split("name=")[1].split(";")[0].split(" ")[0]
                )
            ...
            if b"\r\n\r\n" in part:
                ...
                if (
                    field_name == "video"
                    and b"filename=" in content_disposition.encode()
                ):
                    ...
                    logger.info(
                        f"Found video file: {filename}, size: {len(field_data)} bytes"
                    )
...
def process_video_for_inference(
    video_data: bytes, filename: str, video_format: str = "mp4", user_prompt: str = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        video_info = {"file_size": len(video_data), "format": video_format}
        logger.info(
            f"Processing video: {len(video_data)} bytes, format: {video_format}"
        )

        # Use original video directly - encode as base64 for Bedrock
        logger.info(
            "Encoding video as base64 for direct Bedrock processing (no S3 upload)"
        )
...
def validate_video_format(filename: str, video_data: bytes) -> Dict[str, Any]:
    ...
    SUPPORTED_FORMATS = {
        ...
        ".mpeg": {
            "mime": "video/mpeg",
            "signatures": [b"\x00\x00\x01\xba", b"\x00\x00\x01\xb3"],
        },
        ".mpg": {
            "mime": "video/mpeg",
            "signatures": [b"\x00\x00\x01\xba", b"\x00\x00\x01\xb3"],
    ...
    try:
        ...
        if not signature_found:
            logger.warning(
                f"No expected signature found for {file_extension} format, but proceeding anyway"
            )
        ...
        logger.info(
            f"Video format validation passed: {filename} -> {file_extension} (MIME: {format_info['mime']})"
        )
        return {
            "valid": True,
...
def get_simple_video_info(video_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"file_size": 0, "format": "unknown", "error": str(e)}
...
def parse_basic_video_info(ffmpeg_output: str) -> Dict[str, Any]:
    ...
    try:
        ...
        duration_match = re.search(
            r"Duration: (\d{2}):(\d{2}):(\d{2})\.(\d{2})", ffmpeg_output
        )
        if duration_match:
        ...
        video_match = re.search(
            r"Video: ([^,]+).*?(\d+)x(\d+).*?(\d+(?:\.\d+)?) fps", ffmpeg_output
        )
        if video_match:
...
def create_error_response(status_code: int, message: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "body": json.dumps(
            {"success": False, "error": message, "statusCode": status_code}
        ),
    }
